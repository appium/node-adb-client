...
class USBDevice {
  ...
  async _sendFile(filePath, destination, localId, remoteId) {
    ...
    let dataBuffer = new Buffer(`${devicePath},${stats.mode},DATA${stats.size}${fileData}DONE${modifiedTime}`);
  ...
  async _push (destination, source) {
    ...
    packet = await this._sendAndOkay(ADB_COMMANDS.CMD_WRTE
                                    ...
                                    , remoteId
                                    , destination);
    // this should be a WRTE -> STAT message, not sure what the data in the STAT is
    packet = await this._recvMsg(MAXDATA);
    // reply with an okay
    await this._sendMsg(ADB_COMMANDS.CMD_OKAY
                       , localId
