...
class USBDevice {
  ...
  async _sendFile (filePath, destination, localId, remoteId) {
    ...
    sizeBuffer.writeUInt32LE(stats.size);
    ...
    if (fileSize < 4000) {
      ...
      this._fileLoop(fd, stats, localId, remoteId);
    }
  ...
  async _pull (source, destination) {
    ...
    let dataBuffer = new Buffer(packet.data);
    let length = dataBuffer.readUInt32LE(4);
    // node v0.10.* doesn't support readUIntLE
    let fileData = dataBuffer.slice(8, 8 + length);
    // write data to a local file
    let writeAsync = Promise.promisify(fs.writeFile);
    writeAsync(destination, fileData);
  }
