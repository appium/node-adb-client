...
class USBDevice {
  ...
  _constructFileBuffer (fileData, fileStats, devicePath) {
    ...
    while (remaining > maxSize) {
      sizeBuffer.writeUInt32LE(maxSize);
    ...
    sizeBuffer.writeUInt32LE(remaining);
    ...
    mTimeBuffer.writeUInt32LE(fileStats.mtime.getTime() / 1000);
  ...
  async _sendFile (filePath, destination, localId, remoteId) {
    ...
    // build the entire data buffer
    // let dataBuffer = Buffer.concat([pathBuffer
    //                               , sizeBuffer
    //                               , fileData
    //                               , doneBuffer
    //                               , mTimeBuffer]);
