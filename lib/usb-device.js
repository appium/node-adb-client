// import constants
import { USB_VENDOR_IDS, ADB_VALUES, CONNECT_VALUES,
          ADB_COMMANDS, ADB_KEY } from './constants';
let MAXDATA = CONNECT_VALUES.CONNECT_MAXDATA;
// import helpers
import { generateMessage } from './helpers';
import { packetFromBuffer } from './helpers';

// required libraries
import path from 'path';
import fs from 'fs';
import Promise from 'bluebird';
Promise.longStackTraces();
import * as usb from 'usb';
Promise.promisifyAll(usb);

// local constants
const homedir = (process.platform === 'win32') ? process.env.HOMEPATH
                  : process.env.HOME;
const keyPath = path.join(homedir, ADB_KEY.PUBLIC_KEY);
const publicKeyString = fs.readFileSync(keyPath);

class USBDevice {
  constructor(device, deviceInterface) {
    this.device = device;
    this.deviceInterface = deviceInterface;
  }

  // checks if adb is available, returns either the interface
  // used for adb communication or null if there isn't an
  // appropriate interface for adb communication
  // device parameter is a libusb device
  static getAdbInterface (device) {
    console.log("Checking for ADB.");
    device.open();

    if (device.deviceDescriptor !== null && device.configDescriptor !== null) {
      let vendorID = device.deviceDescriptor.idVendor;

      // if the vendorID is not part of the vendors we recognize
      if (USB_VENDOR_IDS.indexOf(vendorID) === -1) return null;

      let interfaces = device.interfaces;
      let currentInterface = null;
      for (let iface of interfaces) {
        currentInterface = iface;
        // adb interface will only have 2 endpoints
        if (currentInterface.endpoints.length !== 2) continue;
        // interface for ADB always has these values in it's descriptor
        let interfaceDescriptor = currentInterface.descriptor;
        if (interfaceDescriptor.bInterfaceClass !== ADB_VALUES.ADB_CLASS &&
            interfaceDescriptor.bInterfaceSubClass !== ADB_VALUES.ADB_SUBCLASS &&
            interfaceDescriptor.bInterfaceProtocol !== ADB_VALUES.ADB_PROTOCOL) {
          continue;
        }
        currentInterface = iface;
        //if we get to this point we should have the interface we want
        break;
      }
      return currentInterface;
    }
  }

  // loops through all devices found by libusb
  // returns a libusb device that has an ADB interface or null
  static findAdbDevice () {
    let usbDevices = usb.getDeviceList();
    for (let device of usbDevices) {
      let deviceInterface = this.getAdbInterface(device);
      if (deviceInterface !== null) {
        console.log("Found an ADB device.");
        return new USBDevice(device, deviceInterface);
      }
    }
    return null;
  }

  // opens the device and sets up the endpoints for out object
  async claimDevice () {
    console.log("Trying to claim the device.");
    // we need to call these in OSX, see nonolith/nodeusb issue#61
    if (process.platform === 'darwin') {
      this.device.__open();
      this.device.__claimInterface(0);
    }
    this.device.open();
    console.log("device opened");
    this.deviceInterface = Promise.promisifyAll(this.deviceInterface);
    this.deviceInterface.claim();
    await this.deviceInterface.setAltSettingAsync(0);
    let endpoints = this.deviceInterface.endpoints;
    // set the endpoints for our USBDevice object
    // note that these endpoints are on the device, so it's endpoint in
    // is our endpoint out, the endpoint we want to send data to
    if (endpoints[0] === usb.LIBUSB_ENDPOINT_IN) {
      this.inputEndpoint = endpoints[1];
      this.outputEndpoint = endpoints[0];
    } else {
      this.inputEndpoint = endpoints[0];
      this.outputEndpoint = endpoints[1];
    }
    this.inputEndpoint.transferType = usb.LIBUSB_TRANSFER_TYPE_BULK;
    this.outputEndpoint.transferType = usb.LIBUSB_TRANSFER_TYPE_BULK;
    return;
  }

  // handles authentication between our machine and
  // the android device upon initial connection
  // TODO: attempt to sign token sent to us
  async handleAuth () {
    let publicKeyBuf = new Buffer(publicKeyString.length + 1);
    publicKeyString.copy(publicKeyBuf);
    publicKeyBuf[-1] = 0;
    let authMsg = generateMessage(ADB_COMMANDS.CMD_AUTH
                                  , 3
                                  , 0
                                  , publicKeyBuf);
    let command = null;
    do {
      await this.outputEndpoint.transferAsync(authMsg);
      await this.outputEndpoint.transferAsync(publicKeyBuf);
      console.log("sent auth message and public key");
      let data = await this.inputEndpoint.transferAsync(MAXDATA);
      if (data === null) throw new Error("Input transfer returned no data.");
      let packet = packetFromBuffer(Buffer.concat([data]));
      //TODO, actually check the fields of the packet
      console.log("packet command is cnxn? "
                  , packet.command === ADB_COMMANDS.CMD_CNXN);
      if(packet.command === ADB_COMMANDS.CMD_CNXN) {
        // we need to read the string about the device that gets sent back
        await this.inputEndpoint.transferAsync(MAXDATA);
        return;
      }
    } while (command !== ADB_COMMANDS.CMD_CNXN);
  }

  // calls claim device and then performs the ADB usb handshake
  // returns a promise, will have the error value if an error is thrown
  async initConnection () {
    await this.claimDevice();
    console.log("trying to connect to device.");
    this.outputEndpoint = Promise.promisifyAll(this.outputEndpoint);
    this.inputEndpont = Promise.promisifyAll(this.inputEndpoint);
    let connectMsg = generateMessage(ADB_COMMANDS.CMD_CNXN
                                    , CONNECT_VALUES.CONNECT_VERSION
                                    , CONNECT_VALUES.CONNECT_MAXDATA
                                    , CONNECT_VALUES.CONNECT_PAYLOAD);
    let hostBuf = new Buffer(CONNECT_VALUES.CONNECT_PAYLOAD);
    await this.outputEndpoint.transferAsync(connectMsg);
    await this.outputEndpoint.transferAsync(hostBuf);
    console.log("Sent connect message.");
    let data = await this.inputEndpoint.transferAsync(MAXDATA);
    if (data === null) throw new Error("Input transfer returned no data.");
    let packet = packetFromBuffer(Buffer.concat([data]));
    //TODO, actually check the fields of the packet
    data = await this.inputEndpoint.transferAsync(MAXDATA);
    packet.data = data;
    if (packet.command === ADB_COMMANDS.CMD_AUTH) {
      console.log("handle auth");
      await this.handleAuth();
    }
    return;
  }

  // sends a message, checks that message is a Buffer
  async _sendMsg (msg) {
    // this error checking breaks the function :/
    // if(typeof msg !== Buffer) {
    //   throw new Error("Message was not of type Buffer.");
    // }
    return await this.outputEndpoint.transferAsync(msg);
  }

  // receives a message from a device
  // handles reading the adb packet and if necessarry the data payload
  async _recvMsg (amount) {
    let data = await this.inputEndpoint.transferAsync(amount);
    let packet = packetFromBuffer(data);
    if (packet.dataLen > 0) {
      let dataLen = packet.dataLen;
      // let dataPaylad = null
      console.log("packet dataLen is: ", packet.dataLen);
      while (dataLen > 0) {
        console.log("dataLen is: ", dataLen);
        let readData = await this.inputEndpoint.transferAsync(amount);
        console.log("readData is: ", readData.toString());
        dataLen = dataLen - readData.length;
        packet.data = Buffer.concat([packet.data, readData]);
      }
      console.log("loop says we read all, dataLen is: ", dataLen);
      let finalReadData = await this.inputEndpoint.transferAsync(amount);
      packet.data = Buffer.concat([packet.data, finalReadData]);
      //packet.data = await this.inputEndpoint.transferAsync(amount);
    }
    return packet;
  }

  async _shell (commandString) {
    console.log("inside shell function");
    let shellString = "shell:" + commandString + ".";
    console.log("1");
    let openMsg = generateMessage(ADB_COMMANDS.CMD_OPEN
                                  , 12345
                                  , 0
                                  , shellString);
    console.log("2");
    let shellBuf = new Buffer(shellString);
    console.log("3");
    console.log("sending open path");
    await this._sendMsg(openMsg);
    console.log("open sent");
    await this._sendMsg(shellBuf);
    console.log("shell sent");
    // we should get an OKAY here
    let packet = await this._recvMsg(MAXDATA);
    if (packet.command !== ADB_COMMANDS.CMD_OKAY) {
      throw new Error("OPEN response was not OKAY.");
    }
    // this response should be a WRTE followed by a message saying
    // something about process could not be opened :(
    packet = await this._recvMsg(MAXDATA);
    // we need to send an OKAY in order to get the next WRTE message
    let okayMsg = generateMessage(ADB_COMMANDS.CMD_OKAY
                                  , packet.arg2
                                  , packet.arg1
                                  , "");
    await this._sendMsg(okayMsg);
    //we should get the actual data from our shell command now
    packet = await this._recvMsg(MAXDATA);
    console.log(packet.data.toString());
    return;
  }

  async openStream (command) {
    console.log(command.type);
    console.log(command.string);
    switch (command.type) {
      case "shell":
        console.log("shell case");
        await this._shell(command.string);
        break;
      default:
        console.log("default case");
        break;
    }
    return;
  }
}

export default USBDevice;