...
class ADBDevice {
  ...
  async sendAndOkay (cmd, arg1, arg2, payload) {
    console.log("sending: ", payload === ADB_SUBCOMMANDS.CMD_QUIT);
  ...
  async sendFile(filePath, devicePath, fileStats, localId, remoteId) {
  ...
  async push(source, destination) {
    ...
    // get stats from the device about the destination
    ...
    // start sending the actual file data
    await this.sendFile(source, devicePath, stats, localId, remoteId);
    // clean up the connection
    // the device is going to send us a WRTE containing an OKAY as the data
    await this.device._recvMsg(CONNECT_VALUES.CONNECT_MAXDATA);
    ...
    await this.sendAndOkay(ADB_COMMANDS.CMD_WRTE
                          , localId
                          , remoteId
