...
import { getFileName, parseFileData } from './helpers';
...
// let signLib = require('../../build/Release/binding');
...
class ADBDevice {
  ...
  async sendFile (filePath, destination, localId, remoteId) {
    ...
    console.log("File sending is finished, cleanup time");
  ...
  // used for geting the response when we send signed token OR public key
  async waitAuthResponse() {
    let packet = await this.device._recvMsg(CONNECT_VALUES.CONNECT_MAXDATA);
